...
enum class VertexFormat(vararg val elements: Element) {
    ...
    enum class Element(val category: Category, val constant: Int, val count: Int, val length: Int) {
        Position2f(Category.Position, GL_FLOAT, 2, 8),
        Position3f(Category.Position, GL_FLOAT, 3, 12),
        Position2d(Category.Position, GL_DOUBLE, 2, 16),
        Position3d(Category.Position, GL_DOUBLE, 3, 24),
        Position2i(Category.Position, GL_INT, 2, 8),
        Position3i(Category.Position, GL_INT, 3, 12),
        Color(Category.Color, GL_UNSIGNED_BYTE, 4, 4),
        Texture(Category.Texture, GL_FLOAT, 2, 8);
